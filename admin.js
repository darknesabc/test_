...
function getUniversityLineHtml_(placement) {
  ...
  window.renderDepartmentListHelper = function(deptDataList, keyword = "", referenceScore = 0) {
      ...
      const htmlStr = deptDataList.slice(0, limit).map(d => {
          ...
          badges.forEach(b => {
              ...
              else if (b.includes("과탐") || b.includes("사과")) bg = "rgba(52, 152, 219, 0.15)"; 
  ...
  window.renderSingleGroupDataHelper = function(univDataObj, keyword = "", referenceScore = 0) {
    ...
    univKeys.forEach(u => {
      ...
      deptCells += `<td style="border:1px solid rgba(255,255,255,0.1); padding:6px; font-size:11px; vertical-align:top; color:rgba(255,255,255,0.8);">${window.renderDepartmentListHelper(univDataObj[u], keyword, referenceScore)}</td>`;
    });
  ...
  window.runUniversitySimulation = function() {
    ...
    // 💡 [핵심 추가] 패널 하단에 과목별 백분위 띄워주기
    const breakdownEl = document.getElementById('sim-subject-breakdown');
    if (breakdownEl && placeData.scores) {
        const sc = isAvgMode ? placeData.scores.avg : placeData.scores.single;
        const prefix = isAvgMode ? "📊 누적 평균 백분위" : "📊 해당 모평 백분위";
        breakdownEl.innerHTML = `<span style="color:#f39c12; margin-right:8px;">${prefix}</span> 
                                 국어 <span style="color:#3498db; font-size:13px;">${sc.k}</span> <span style="opacity:0.4; margin:0 4px;">|</span> 
                                 수학 <span style="color:#e74c3c; font-size:13px;">${sc.m}</span> <span style="opacity:0.4; margin:0 4px;">|</span> 
                                 탐구 <span style="color:#2ecc71; font-size:13px;">${sc.t}</span>`;
    }
    ...
    placeData.allMatches.forEach(m => {
      ...
      if (keyword) {
          ...
          if (targetScore > 0 && m.score >= targetScore - 1 && m.score <= targetScore + 2) isUpMatch = true;
      }

      if (isUpMatch) {
          if (!upLines[m.gun][m.univ]) upLines[m.gun][m.univ] = [];
          upLines[m.gun][m.univ].push(item);
      }
    });
            ...
            <div style="background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.08); border-radius: 12px; padding: 15px; margin-bottom: 12px; transition:all 0.2s;" onmouseover="this.style.background='rgba(255,255,255,0.06)'" onmouseout="this.style.background='rgba(255,255,255,0.03)'">
                <div style="display:flex; justify-content:space-between; align-items:flex-start; margin-bottom:10px; gap:10px; flex-wrap:wrap;">
                    <div style="flex:1; min-width:200px;">
                        ...
                        <div style="color:rgba(255,255,255,0.5); font-size:11px; margin-top:4px;">
                            ${r.qual ? `<b>지원자격:</b> ${escapeHtml(r.qual)} ` : ""}
                            ${r.slots ? `(모집: ${escapeHtml(r.slots)}명)` : ""}
                        </div>
                    </div>
                ...
                <div style="display:grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap:10px; background:rgba(0,0,0,0.3); padding:10px 12px; border-radius:8px; font-size:11px;">
                    <div style="border-right:1px solid rgba(255,255,255,0.1); padding-right:10px;">
                        ...
                        <div style="color:#fff; line-height:1.4; word-break:keep-all;">${escapeHtml(r.method)}</div>
