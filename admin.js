...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function renderAttendanceDetail_(data, moveMap) {
    ...
    return `<div style="overflow:auto; border-radius:14px; border:1px solid rgba(255,255,255,.08);"><table style="width:max-content; min-width:100%; border-collapse:separate; border-spacing:0; font-size:14px;"><thead style="background: rgba(255,255,255,.03);"><tr>${thTop}</tr><tr>${thSub}</tr></thead><tbody>${bodyTr || `<tr><td style="padding:12px; opacity:.8;" colspan="${1 + lastIdx.length*2}">데이터 없음</td></tr>`}</tbody></table></div>`;
  }
